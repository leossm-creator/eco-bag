...
var Lion = function () {
  ...
  let positionAttribute = this.body.geometry.getAttribute('position');

  for (let i = 0; i < this.bodyVertices.length; i++) {
    // var tv = this.body.geometry.vertices[this.bodyVertices[i]];
    // console.log(this.body.geometry.attributes.position)

    // var tv = this.body.geometry.attributes.position[this.bodyVertices[i]];
    // console.log(this.body.geometry.attributes.position)
    //tv.x = 0;

    var tv = vertex.fromBufferAttribute(positionAttribute, i);
    tv.z = 70;
...
Lion.prototype.look = function (xTarget, yTarget) {
  ...
  for (let i = 0; i < this.bodyVertices.length; i++) {
    ...
    // this.bodyInitPositions.push({ x: tv.x, y: tv.y, z: tv.z });
    // var tv = this.body.geometry.vertices[this.bodyVertices[i]];
    var tv = vertex.fromBufferAttribute(positionAttribute, this.bodyVertices[i]);
